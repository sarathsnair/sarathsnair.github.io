    ...
    >
        ...
        <div className="flex items-start justify-between mb-3 md:mb-4">
          <div className="flex-shrink-0">
            <div className="w-16 h-16 md:w-24 md:h-24 border-3 md:border-4 border-black p-1.5 md:p-2 bg-white">
              <Image
                src={exp.logo}
          ...
          {exp.url && (
            <Link
              ...
              className="group/link w-10 h-10 flex items-center justify-center border-2 border-black bg-white text-black hover:bg-black hover:text-white transition-colors"
            >
        ...
        <div className="space-y-2 md:space-y-3">
          ...
          {/* Description */}
          {exp.description && (
            <p className="text-base md:text-lg leading-[1.7] text-black" style={{ fontWeight: 500 }}>
                {exp.description}
              </p>
            )}
              ...
              <ul className="space-y-2">
                {exp.responsibilities.slice(0, 3).map((resp: string, i: number) => (
                  <li key={i} className="flex items-start gap-3">
                    <div className="w-2 h-2 bg-black flex-shrink-0 mt-2" />
                    <span className="text-sm md:text-base leading-[1.7] text-black">{resp}</span>
                  </li>
                ))}
              </ul>
            </div>
